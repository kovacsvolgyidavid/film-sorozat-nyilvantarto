...
../glassfish/glassfish4/bin/asadmin --echo --user admin --passwordfile passwords.txt create-jms-resource --restype javax.jms.Topic jms/CreateJMSEntity
../glassfish/glassfish4/bin/asadmin --echo --user admin --passwordfile passwords.txt create-jms-resource --restype javax.jms.Topic jms/DeleteJMSEntity

#################################################

#Create JDBCRealm
../glassfish/glassfish4/bin/asadmin --echo --user admin --passwordfile passwords.txt create-auth-realm --classname com.sun.enterprise.security.auth.realm.jdbc.JDBCRealm --property="jaas-context=jdbcRealm:datasource-jndi=jdbc\\/movie:user-table=users:user-name-column=username:password-column=password:group-table=groups:group-name-column=groups:group-table-user-name-column=username:digestrealm-password-enc-algorithm=AES:charset=UTF-8:digest-algorithm=SHA-256" SecurityJDBCRealm
